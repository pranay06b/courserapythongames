#RiceRocks (Asteroids)
# implementation of Spaceship - program template for Rice
...
def rock_spawner():
    ...
    if len(rock_group)<12 and started:
        ...
        spawn_dist=dist(my_ship.pos,a_rock.pos)
        if spawn_dist>50:
...
# check for colisions between two groups
def group_group_collide(rocks, missiles):
#    pass
    j=0
    
    for i in rocks:
        if group_collide(missiles,i)==True:
            j+=1
            rocks.remove(i)
